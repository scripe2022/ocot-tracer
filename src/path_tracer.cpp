...
// 4b2 run  := time ./raytracer scenes/test4/dragon.test && kcat output/dragon.png
...
// tiny run := time ./raytracer scenes/tiny/cornellBRDF.test && kcat output/cornellBRDF.png
...
std::vector<glm::vec3> direct_lighting_gpu(std::vector<Ray> &rays, std::vector<hitobj> &hits) {
    ...
    for (auto light: scene.lights) {
        ...
        glm::vec3 n_l = glm::normalize(glm::cross(quad.ac, quad.ab));
        float area = glm::length(glm::cross(quad.ab, quad.ac));
        std::vector<std::pair<float, float>> samples = gen_sample(N, false);
        std::vector<glm::vec3> hitpos_vec, xp_vec;
        for (uint i = 0; i < N; ++i) {
            float da = samples[i].first, db = samples[i].second;
            glm::vec3 xp = quad.a + da * quad.ab + db * quad.ac;
            glm::vec3 hitpos = get_hitpos(rays[i], hits[i]);
            xp_vec.push_back(xp);
        ...
        for (uint i = 0; i < N; ++i) {
            ...
            glm::vec3 hitpos = hitpos_vec[i];
            glm::vec3 xp = xp_vec[i];
            glm::vec3 w_o = glm::normalize(rays[i].origin - hitpos);
            glm::vec3 w_i = glm::normalize(xp - hitpos);
            glm::vec3 f = brdf(hits[i], w_i, w_o);
            float g = gterm(hitpos, hits[i].normal, xp, n_l);
            float p = 1.0f;
            if (scene.nee == "mis") {
                float p_nee = glm::pow(get_p_nee(hitpos, w_i), 2);
                float p_brdf = glm::pow(pdf(hits[i], w_i, w_o), 2);
                p = p_nee / (p_nee + p_brdf);
            }
            colors[i] += area * quad.intensity * f * g * p;
        }
    }
    if (scene.nee == "mis") {
...
glm::vec3 trace(Ray &ray, uint depth) {
    ...
    if (scene.nee == "on" || scene.nee == "mis") {
        if (is_light_source) {
            glm::vec3 surface_normal = std::get<Triangle>(hit.shape).surface_normal;
            if (!depth && glm::dot(surface_normal, ray.direction) >= 0.0f) color += emission;
            else return color;
        }
        if (!scene.rr && depth >= scene.maxdepth) return color;
...
std::vector<glm::vec3> trace_gpu(std::vector<Ray> &rays, uint depth) {
    ...
    for (uint i = 0; i < rays.size(); ++i) {
        ...
        if (scene.nee == "on" || scene.nee == "mis") {
            if (is_light_source) {
                glm::vec3 surface_normal = std::get<Triangle>(hit.shape).surface_normal;
                if (!depth && glm::dot(surface_normal, ray.direction) >= 0.0f) colors[i] += emission;
                else continue;
            }
            if (!scene.rr && depth >= scene.maxdepth) continue;
        }
        else if (is_light_source || (!scene.rr && depth > scene.maxdepth)) {
            colors[i] = emission;
            continue;
        }
    ...
    if (remained_rays.empty()) return colors;
    if (scene.nee == "on" || scene.nee == "mis") {
        std::vector<glm::vec3> nee_colors = direct_lighting_gpu(remained_rays, remained_hits);
        for (uint i = 0; i < nee_colors.size(); ++i) colors[remained_idx[i]] += nee_colors[i];
    }
    std::vector<Ray> new_rays;
    std::vector<uint> new_idx;
    ...
    for (uint idx = 0; idx < remained_rays.size(); ++idx) {
        ...
        new_idx.push_back(remained_idx[idx]);
        coefs.push_back(coef * T);
    }
    std::vector<glm::vec3> new_colors = trace_gpu(new_rays, depth + 1);
    for (uint i = 0; i < new_colors.size(); ++i) {
        colors[new_idx[i]] += new_colors[i] * coefs[i];
    }
    return colors;
}
...
uint cnt_gpu = 0;
void render_gpu() {
    std::vector<glm::vec3> final_colors(scene.width * scene.height, glm::vec3(0.0f));
    for (uint k = 0; k < scene.spp; ++k) {
        ...
        for (uint i = 0; i < scene.height; ++i) for (uint j = 0; j < scene.width; ++j) {
            ...
            rays.push_back(ray_thru_pixel(scene, (float)i + da, (float)j + db));
        }
        std::vector<glm::vec3> colors = trace_gpu(rays, 0);
        for (uint i = 0; i < scene.height; ++i) for (uint j = 0; j < scene.width; ++j) {
            final_colors[i * scene.width + j] += colors[i * scene.width + j];
        }
    }
    for (uint i = 0; i < scene.height; ++i) for (uint j = 0; j < scene.width; ++j) {
        img.set(i, j, final_colors[i * scene.width + j] / (float)scene.spp);
    }
}
...
Image run(Scene &s, int mode, bool show) {
    ...
    if (mode == 0) {
        ...
        uint sh = 47;
        uint sw = 240;
        for (uint i = sh; i < sh + 1; ++i) {
            for (uint j = sw; j < sw + 1; ++j) {
